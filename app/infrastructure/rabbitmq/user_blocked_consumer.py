...
from app.infrastructure.rabbitmq.dto_parser_user_blocked import parse_user_blocked_event
...
_consumer_name = "user-blocked-consumer"
_queue_name = ""


async def _user_blocked_message_handler(
    ...
    async with message.process(requeue=False):
        try:
            event = parse_user_blocked_event(message.body)
            if _metrics is not None:
                _metrics.inc_rabbitmq_message(_consumer_name, _queue_name, "received")

            async with container() as scope:
                use_case = await scope.get(UpdateIsActiveUserUseCase)
                result = await use_case.execute(
                    user_id=event.user_id, is_blocked=event.is_blocked
                )
...
def get_consumer_specs() -> list[ConsumerSpec]:
    ...
    queue = rabbit.queue_user_blocked
    return [
        ConsumerSpec(
            name=_consumer_name,
            exchange="user",
